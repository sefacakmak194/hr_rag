...
import type { PolicyGap, PolicyGapResponse } from '../types';
...
export default function PolicyGapPanel() {
  const [data, setData] = useState<PolicyGapResponse | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData((await res.json()) as PolicyGapResponse);
      setError(null);
    } catch (e) {
    ...
    const lines = [
      ...
      ...data.gaps.map((g) =>
        [
          g.label,
          g.count,
          g.topScore.toFixed(3),
          g.nearMiss ? 'evet' : 'hayir',
          g.questions.map((q) => q.text).join(' | '),
        ]
          .map(esc)
          .join(','),
      ),
      ...
      <header className="view-head">
        <div>
          ...
          <h2 className="view-title">
            {data
              ? `${data.totalUnanswered} yanıtsız soru · ${data.gaps.length} konu · ${weeks.length} hafta`
              : 'Politika boşlukları'}
          </h2>
        </div>
        <div className="view-actions">
          ...
          <button type="button" className="btn" onClick={exportCsv} disabled={!data?.gaps.length}>
            CSV indir
          </button>
        </div>
      </header>
  ...
  return (
    <div className={`gap${open ? ' gap--open' : ''}`}>
      <button type="button" className="gap-head" onClick={onToggle}>
        ...
        <span className="gap-score">{gap.topScore.toFixed(3)}</span>
      </button>
